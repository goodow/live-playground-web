...
import WrappedOperation from './wrapped-operation.js';
...
firepad.RichTextQuillAdapter = (function () {
  ...
  RichTextQuillAdapter.prototype.getCursor = function () {
    ...
    return range === null ? null : new Cursor(range.index, range.index + range.length);
  };

  RichTextQuillAdapter.prototype.setCursor = function (cursor) {
    this.quill.setSelection(cursor.position, cursor.selectionEnd - cursor.position, 'silent');
  };
  ...
  RichTextQuillAdapter.prototype.applyOperation = function (operation) {
    this.quill.updateContents(operation, 'silent');
  };
  ...
  Delta.prototype.equals = function (other) {
    ...
    for (var i = 0; i < this.ops.length; i++) {
      if (typeof this.ops[i]['delete'] === 'number') {
        if (this.ops[i]['delete'] !== other.ops[i]['delete']) {
          return false;
        }
      } else if (typeof this.ops[i].retain === 'number') {
        if (this.ops[i].retain !== other.ops[i].retain || !attributesEqual(this.ops[i].attributes, other.ops[i].attributes)) {
          return false;
        }
      } else if (typeof this.ops[i].insert === 'string') {
        if (this.ops[i].insert !== other.ops[i].insert || !attributesEqual(this.ops[i].attributes, other.ops[i].attributes)) {
          return false;
        }
      } else {
        var embed = Object.keys(this.ops[i].insert)[0];
        if (typeof this.ops[i].insert[embed] !== 'object') {
          if (this.ops[i].insert[embed] !== other.ops[i].insert[embed] || !attributesEqual(this.ops[i].attributes, other.ops[i].attributes)) {
            return false;
          }
        } else {
          if (!attributesEqual(this.ops[i].insert[embed], other.ops[i].insert[embed])
            || !attributesEqual(this.ops[i].attributes, other.ops[i].attributes)) {
            return false;
          }
        }
      }
  ...
  function attributesEqual(attributes1, attributes2) {
    if ((attributes1 === undefined || attributes2 === undefined) && attributes1 !== attributes2) {
      return false;
    }
...
export default firepad.RichTextQuillAdapter;
